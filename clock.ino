...
const int t =4;
const int LEDbrightness_step = 40;
const int n_color = 135;

Color arr_color[n_color] = { {255, 250, 250},
                      {248, 248, 255},
                      {245, 245, 245},
                      {220, 220, 220},
                      {255, 250, 240},
...
uint8_t m_hour[2], m_minute[2], m_day[2], m_month[2], m_temp[2], arr_time_delay[2], arr_date_delay[2], arr_temp_delay[2];
...
bool save_LEDbrightness_flag = false;
...
void loop() {
    ...
    if (save_LEDbrightness_flag) {
      if (millis() - time_save_LEDbrightness >= 10000) {
        save_LEDbrightness_flag = false;
        EEPROM.write(7, LEDbrightness);
      }
    }                 
}
...
void brightness() {
    if (brightness_auto) {
      ...
      if (abs(prev_photocellReading - photocellReading) > LEDbrightness_step) {
        if (photocellReading < 160) photocellReading = 160;
        if (photocellReading > 650) photocellReading = 650;
...
void setMyTime() {
    ...
    if (fix_temp) {
      switch (cmp_temp) {
        case 1 : { if (millis() > 345000) {
                      add_temp = -1;
                      cmp_temp = 2;
                    }
                    break;
        }
        case 2 : { if (millis() > 601000) {
                      add_temp = -2;
                      cmp_temp = 3;
                  }
                  break;
        }
        case 3 : { if (millis() > 793000) {
                      add_temp = -3;
                      cmp_temp = 4;
                    }
                    break;           
        }
        case 4 : { if (millis() > 1049000) {
                      add_temp = -4;
                      cmp_temp = 5;
                    }
                    break;
        }
        case 5 : { if (millis() > 1369000) {
                      add_temp = -5;
                      cmp_temp = 6;
                    }
                    break;
        }
        case 6 : { if (millis() > 1817000) {
                      add_temp = -6;
                      cmp_temp = 7;
                    }
                    break;
        }
        case 7 : { if (millis() > 2713000) {
                      add_temp = -7;
                      cmp_temp = 0;
                      fix_temp = false;
                    }
...
void check(int x) {
  ...
  if (x == 0x1E) {
    if (index_color < n_color) index_color++;
    else index_color = 0;
    user_color = arr_color[index_color];
    save_index_color_flag = true;
    time_save_index_color = millis();
  ...
  if (x == 0xB) {
    color_flag = 3;
    if (user_color.blue > 0) user_color.blue--;
    return;
  }
  if (x == 0xC) {
    color_flag = 3;
    if (user_color.blue < 255) user_color.blue++;
    return;
  }
  if (x == 0x5) {
    color_flag = 1;
    if (user_color.red > 0) user_color.red--;
    return;
  }
  if (x == 0x7) {
    color_flag = 1;
    if (user_color.red < 255) user_color.red++;
    return;
  }
  if (x == 0x9) {
    color_flag = 2;
    if (user_color.green > 0) user_color.green--;
    return;
  }
  if (x == 0xA) {
    color_flag = 2;
    if (user_color.green < 255) user_color.green++;
    return;
  }
  if (x == 0xE) {
    mode = 1;
...
void save_time() {
   now = Rtc.GetDateTime();
   Rtc.SetDateTime(RtcDateTime(now.Year(), now.Month(), now.Day(), (uint8_t) (m_hour[1] * 10 + m_hour[0]), (uint8_t) (m_minute[1] * 10 + m_minute[0]), now.Second()));
}


void save_date() {
   now = Rtc.GetDateTime();
   Rtc.SetDateTime(RtcDateTime(now.Year(), (m_month[1] * 10 + m_month[0]), (m_day[1] * 10 + m_day[0]), now.Hour(), now.Minute(), now.Second()));
}
